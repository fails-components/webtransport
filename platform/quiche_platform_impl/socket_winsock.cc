...
absl::Status SetSocketMode(SocketFd fd, unsigned long mode) {
  QUICHE_DCHECK_GE(fd, 0);
  ...
  if (ret == SOCKET_ERROR) {
    absl::Status status = ToStatus(WSAGetLastError(), "WSAIoctl");
    QUICHE_LOG_FIRST_N(ERROR, 100)
        << "Could not set mode for socket " << fd << " with error: " << status;
    return status;
  }
  return absl::OkStatus();
}
...
absl::Status SetSockOptInt(SocketFd fd, int option, int value) {
  ...
  int result;
  do {
    result = ::setsockopt(fd, SOL_SOCKET, option,reinterpret_cast<const char *>(&value), sizeof(value));
...
absl::Status SetSocketBlocking(SocketFd fd, bool blocking) {
  if (blocking) {
    return SetSocketMode(fd, 0);
  } else {
    return SetSocketMode(fd, 1);
  }
}
...
absl::Status GetSocketError(SocketFd fd) {
  ...
  do {
    sockopt_result =
        ::getsockopt(fd, SOL_SOCKET, SO_ERROR, reinterpret_cast<char *> (&socket_error), &len);
...
absl::Status Close(SocketFd fd) {
  ...
  int close_result = ::closesocket(fd);
