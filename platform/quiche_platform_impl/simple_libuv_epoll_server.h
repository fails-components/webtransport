...
#ifndef QUICHE_LIBUV_SERVER_H_
#define QUICHE_LIBUV_SERVER_H_
...
struct LibuvEpollEvent {
  LibuvEpollEvent(int events) : in_events(events), out_ready_mask(0) {}
...
// class.
class LibuvEpollCallbackInterface {
 public:
  // Summary:
  ...
  //                in the epoll() calls)
  virtual void OnRegistration(SimpleLibuvEpollServer* eps, int fd,
                              int event_mask) = 0;
  ...
  virtual void OnEvent(int fd, LibuvEpollEvent* event) = 0;
  ...
  // Args:
  //  fd - the file descriptor which was registered.
  virtual void OnShutdown(SimpleLibuvEpollServer* eps, int fd) = 0;
...
class SimpleLibuvEpollServer {
 public:
  ...
  SimpleLibuvEpollServer(const SimpleLibuvEpollServer&) = delete;
  SimpleLibuvEpollServer operator=(const SimpleLibuvEpollServer&) = delete;

  ////////////////////////////////////////

  // Destructor
  virtual ~SimpleLibuvEpollServer();
  ...
  virtual void UnregisterAlarm(
      const SimpleLibuvEpollServer::AlarmRegToken& iterator_token);

  virtual SimpleLibuvEpollServer::AlarmRegToken ReregisterAlarm(
...
// callbacks for libuv
  ...
  struct CBAndEventMask {
    ...
    CBAndEventMask(LibuvEpollCallbackInterface* cb, int event_mask, int fd)
        : cb(cb),
          fd(fd),
  ...
  virtual void DelFD(int fd, uv_poll_t *handle) const;
  ...
  //                 FD initially.
  virtual void AddFD(int fd, uv_poll_t *handle, int event_mask) const;
  ...
  //                 OR'd together) which will be associated with this
  //                 FD after this call.
  virtual void ModFD(int fd, uv_poll_t *handle, int event_mask) const;
...
class LibuvEpollAlarmCallbackInterface {
 public:
  ...
  virtual void OnRegistration(const SimpleLibuvEpollServer::AlarmRegToken& token,
                              SimpleLibuvEpollServer* eps) = 0;
  ...
  //   registered.
  virtual void OnShutdown(SimpleLibuvEpollServer* eps) = 0;

  virtual ~LibuvEpollAlarmCallbackInterface() {}

 protected:
  LibuvEpollAlarmCallbackInterface() {}
};
...
class LibuvEpollAlarm : public LibuvEpollAlarmCallbackInterface {
 public:
  ...
  void OnRegistration(const SimpleLibuvEpollServer::AlarmRegToken& token,
                      SimpleLibuvEpollServer* eps) override;

  // Marks the alarm as unregistered.
  void OnUnregistration() override;

  // Marks the alarm as unregistered.
  void OnShutdown(SimpleLibuvEpollServer* eps) override;
  ...
  const SimpleLibuvEpollServer* eps() const { return eps_; }

 private:
  SimpleLibuvEpollServer::AlarmRegToken token_;
  SimpleLibuvEpollServer* eps_;
  bool registered_;
};
