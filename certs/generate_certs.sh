...
try openssl pkcs8 \
  ...
  -nocrypt
# Generate the leaf certificate to be valid for 365 days.
try openssl ca \
  -batch \
  -days 365 \
  ...
  # TODO
try openssl x509 -in out/leaf_cert.pem -pubkey -noout | openssl pkey -pubin -outform der | openssl dgst -sha256 -binary | openssl enc -base64 > out/fingerprint.txt
