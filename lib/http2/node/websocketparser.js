...
function applyMask(ms, buffer, offset, length) {
  ...
  /* const dwmask = new Uint32Array(workmask.buffer)
  const dwdata = new Uint32Array(buffer.buffer, offset, length / 4)
  
  for (let run = 0; run < length / 4; run++) {
    console.log(
      'dw run',
      dwdata[run].toString(16),
      dwmask[0].toString(16),
      (dwdata[run] ^ dwmask[0]).toString(16)
    )
    dwdata[run] = dwdata[run] ^ dwmask[0]
  } */
  const data = new Uint8Array(buffer.buffer, offset, length)
  for (let run = 0 /* Math.round(length / 4) * 4 */; run < length; run++) {
    data[run] = data[run] ^ workmask[run % 4]
  }
  ms.offset += length
}
/*
