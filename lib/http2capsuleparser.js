import { Http2WebTransportStream } from './http2webtransportstream'
...
function writeVarInt(bs, int) {
  ...
  bs.buffer.writeUInt8(bs.offset, msb | ((int >>> ((numbytes - 1) * 8)) & 0xff))
  bs.offset++

  for (let i = numbytes - 2; i > 0; i--) {
    bs.buffer.writeUInt8(bs.offset, msb | ((int >>> (i * 8)) & 0xff))
    bs.offset++
  }
}
...
export class Http2CapsuleParser {
  ...
  static WT_STREAM_WFIN = 0x190b4d3b
        ...
        case 's':
          {
            ...
            if (!type || bufferstate.size < 1 + bufferstate.offset) {
              this.saveddata = cdata
              return
            }
              ...
              case Http2CapsuleParser.WT_STREAM_WFIN:
                ...
                if (streamid) {
                  ...
                  if (!object) {
                    ...
                    this.wtstreams.set(streamid, object)
                  }
                  // TODO submit data
                  object.recvData({
                    data: new Uint8Array(
