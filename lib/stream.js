...
const log = logger('webtransport:http3wtstream')
...
export class Http3WTStream {
    ...
    switch (args.nettask) {
      case 'resetStream':
        if (this.readable) {
          ...
          log.error('stopSending without readable')
        }
        break
  ...
  static callback(args) {
    log('Stream callback called', args?.purpose)
    log.trace(log)
      ...
      switch (args.purpose) {
        ...
        case 'StreamRead':
          if (visitor && Object.prototype.hasOwnProperty.call(args, 'data')) {
            ...
            log('Stream callback called', visitor, args)
